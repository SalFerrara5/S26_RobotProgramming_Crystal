...
class TurtleClient(Node):
    def __init__(self):
        ...
        #### parameter for setting color ####
        self.declare_parameter('turtleColor', 'blue', ParameterDescriptor(description='Defines the color of the turtle [not the trail] on startup.'))
        
        #### reads parameter value and sets the turtle to that color ####
        turtleColor = self.get_parameter('turtleColor').get_parameter_value().string_value
        self.turtle_display.color(turtleColor)
	
	#### allocates space for color parameter to be read with a service call ####
        ...
        #### parameter for setting pen size ####
        self.declare_parameter('penSize', 2, ParameterDescriptor(description='Defines the pen thickness of the turtle trail on startup.'))

        #### reads parameter value and sets the turtle pen size ####
