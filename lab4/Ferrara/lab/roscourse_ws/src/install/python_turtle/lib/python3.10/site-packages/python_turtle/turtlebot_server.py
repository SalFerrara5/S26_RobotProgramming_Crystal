...
class TurtlebotServer(Node):
	...
	#color
	self.turtle_color_srv = self.create_service(<service type>, 'set_Color', self.set_color_callback)
    ...
    def set_color_callback(self,request,response):
    	self.turtle.color = request._____ #fill in this blank
    	self.get_logger().info('Turtle color set: %s' %(self.turtle.____)) #fill in the blank
    	response.ret = 1
    	return response
