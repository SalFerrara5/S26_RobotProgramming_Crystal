...
class TurtleSquareClient(Node):
    def __init__(self):
        ...
        # Declare square_size as a parameter
        square_size_descriptor = ParameterDescriptor(
            description='Size of the square for the turtle to draw'
        )

        self.declare_parameter(
            'square_size',
            50.0,
            square_size_descriptor
        )
    ...
    def get_result_callback(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            self.get_logger().info(
                'Goal succeeded! Result: {0}'.format(result.final_pose)
            )
        else:
    ...
    def send_goal(self):
        ...
        # Get square_size from parameter
        goal_msg.square_size = self.get_parameter(
            'square_size'
        ).get_parameter_value().double_value

        self.get_logger().info(
            f'Sending goal request with square_size = {goal_msg.square_size}'
        )
